...
/*   Updated: 2025/11/07 16:18:10 by ybassour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	apply_inputs(t_player *player, char **map)
{
	...
	(1) && (pos = player->pos, angle = player->angle, right = player->go_right);
	new_pos.x = pos.x + (((cos(angle) * player->go_up) + \
						(cos(angle + (M_PI / 2)) * right)) * MOVE_SPEED);
	new_pos.y = pos.y + (((sin(angle) * player->go_up) + \
						(sin(angle + (M_PI / 2)) * right)) * MOVE_SPEED);
	player->pos = check_for_collision(map, pos, new_pos, PLAYER_RADIUS);
	if (player->rotate_right || player->rotate_right_mouse)
	{
