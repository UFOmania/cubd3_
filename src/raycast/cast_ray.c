...
static t_vec2	get_h_intercept(t_vec2 pos, double angle, t_frame *frame)
{
	...
	{
		if (a > 0) //looking down
			h_intercept.y = (floor(pos.y / 80) * 80) + 80;
		else 
			h_intercept.y = floor(pos.y / 80) * 80 - 0.00001; //-0.00001 is a trik for marjen
...
static t_vec2	get_v_intercept(t_vec2 pos, double angle, t_frame *frame)
{
	...
	{
		if (a > 0)//facing right
			v_intercept.x = (floor(pos.x / 80) * 80) + 80;
		else
			v_intercept.x = floor(pos.x / 80) * 80 - 0.00001;
