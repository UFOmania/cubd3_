...
char    *is_valid_texture(char *line, const char *id)
{
	...
	if (!line || !id)
		return (display_error_message(ERR_NULL_INPUT, NULL), NULL);
	if (ft_strncmp(line, id, ft_strlen(id)) != 0
		|| line[ft_strlen(id)] != ' ')
			return (display_error_message(ERR_INVALID_ID, NULL), NULL);
	i = ft_strlen(id);
	while (line[i] == ' ')
		i++;
	...
	if (fd == -1)
	{
		display_error_message(ERR_FILE_OPEN_FAILED, path);
		free(path);
		return (NULL);
	}
	close(fd);
