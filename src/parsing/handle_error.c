...
int	display_error_message(int code, const char *target)
{
	display_error_file( code, target);
	display_error_arg( code);
	display_error_input_malloc( code, target);
	display_error_color( code);
	display_error_direction( code);
	return 0;
}