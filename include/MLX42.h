...
/*   Updated: 2025/08/30 11:26:18 by ybassour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					mlx_get_monitor_size(int32_t index, int32_t* width, \
							int32_t* height);
void					mlx_set_window_pos(mlx_t* mlx, int32_t xpos, \
							int32_t ypos);
void					mlx_get_window_pos(mlx_t* mlx, int32_t* xpos, \
							int32_t* ypos);
void					mlx_set_window_size(mlx_t* mlx, int32_t new_width, \
							int32_t new_height);
void					mlx_set_window_limit(mlx_t* mlx, int32_t min_w, \
							int32_t min_h, int32_t max_w, int32_t max_h);
...
void					mlx_scroll_hook(mlx_t* mlx, mlx_scrollfunc func, \
							void* param);
void					mlx_mouse_hook(mlx_t* mlx, mlx_mousefunc func, \
							void* param);
void					mlx_cursor_hook(mlx_t* mlx, mlx_cursorfunc func, \
							void* param);
void					mlx_key_hook(mlx_t* mlx, mlx_keyfunc func, void* param);
void					mlx_close_hook(mlx_t* mlx, mlx_closefunc func, \
							void* param);
void					mlx_resize_hook(mlx_t* mlx, mlx_resizefunc func, \
							void* param);
bool					mlx_loop_hook(mlx_t* mlx, void (*f)(void*), \
							void* param);
...
mlx_image_t*			mlx_new_image(mlx_t* mlx, uint32_t width, \
							uint32_t height);
int32_t					mlx_image_to_window(mlx_t* mlx, mlx_image_t* img, \
							int32_t x, int32_t y);
void					mlx_delete_image(mlx_t* mlx, mlx_image_t* image);
bool					mlx_resize_image(mlx_image_t* img, uint32_t nwidth, \
							uint32_t nheight);
void					mlx_set_instance_depth(mlx_instance_t* instance, \
							int32_t zdepth);
mlx_image_t*			mlx_put_string(mlx_t* mlx, const char* str, \
							int32_t x, int32_t y);
